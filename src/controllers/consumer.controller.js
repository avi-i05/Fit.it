...
const registerConsumer = asyncHandler(async (req, res) => {
    ...
    // validate - not empty
    if (
        [displayName, email, password, phoneNumber, gender].some(field => field.trim() === "")
    ) {
    ...
    if(consumerExist){
        throw new ApiError(409, "Consumer with given email or phone number already exists");
    }
    ...
    const createdConsumer = await Consumer.findById(consumer._id).select('-password -refreshToken');
...
const loginConsumer = asyncHandler(async(req, res)=> {
    ...
    const loggedInConsumer = await Consumer.findById(consumer?._id).select('-password -refreshToken');
...
const updateConsumerProfile = asyncHandler(async(req, res) => {
    ...
    if(
        [displayName, email, phoneNumber, gender].some(field => !field)
    ){
        throw new ApiError(400, "All fields are required");
    }

    const consumer = await Consumer.findByIdAndUpdate(req.consumer?._id, {
        $set:{
            displayName,
            email,
            phoneNumber,
...
const registerConsumerAddress = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(
        new ApiResponse(201, address, "Address registered successfully")
    );
})

const getConsumerAddresses = asyncHandler(async(req, res) =>{
    const consumer = await Consumer.findById(req.consumer?._id).populate('address');
    ...
    return res.status(200).json(
        new ApiResponse(200, consumer.address, "Consumer addresses fetched successfully")
    );
})
...
const deleteConsumerProfile = asyncHandler(async(req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, null, "Consumer deleted successfully")
    );
})
